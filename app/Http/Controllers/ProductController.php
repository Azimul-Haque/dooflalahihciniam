...
class ProductController extends Controller
{
    ...
    public function searchProducts($search_param) 
    {
      ...
      //                    ->paginate(10);
          ...
          $query->whereHas('categories', function ($q) use ($search_param) {
              $q->where('name', 'like', '%' . $search_param . '%');
          });

          // Search by subcategory name
          $query->whereHas('subcategories', function ($q) use ($search_param) {
              $q->where('name', 'like', '%' . $search_param . '%');
          });
    ...
    public function postCheckout(Request $request) {
      ...
      try{
        ...
        // apatoto payment_method off 
        // apatoto payment_method off 
        // apatoto payment_method off 
        // $order->payment_method = $request->payment_method; // 0 means cash on delivery, 1 means bKash
        $order->deliverylocation = $request->deliverylocation; // 0 == Address, 1020 = Shop
      ...
      if($request->payment_method == 1) {
        Session::flash('success', 'Payment Gatway to be integrated...');
      }
